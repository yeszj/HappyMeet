...
public class CustomEaseChatLayout extends RelativeLayout implements CustomIChatLayout, IHandleMessageView, IPopupWindow
        , ChatInputMenuListener, EMMessageListener, CustomEaseChatMessageListLayout.OnMessageTouchListener
        , MessageListItemClickListener, CustomEaseChatMessageListLayout.OnChatErrorListener {
    private static final String TAG = CustomEaseChatLayout.class.getSimpleName();
    ...
    public EaseHandleMessagePresenter getPresenter() {
        return presenter;
    }
    ...
    public CustomEaseChatLayout(Context context, AttributeSet attrs) {
    ...
    private void initListener() {
        ...
        try {
            if (getChatManager() != null) {
                ...
                getChatManager().addConversationListener(emConversationListener);
            }
    ...
    private final EMConversationListener emConversationListener = new EMConversationListener() {
        @Override
    ...
    public void removeListener() {
        if (getChatManager() != null) {
            ...
            getChatManager().removeConversationListener(emConversationListener);
        }
    ...
    public void init(String username, int chatType) {
        init(CustomEaseChatMessageListLayout.LoadDataType.LOCAL, username, chatType);
    }
     ...
     * @param chatType       聊天类型，单聊，群聊或者聊天室
     */
    public void init(CustomEaseChatMessageListLayout.LoadDataType loadDataType, String conversationId, int chatType) {
    ...
    public void initHistoryModel(String toChatUsername, int chatType) {
        init(CustomEaseChatMessageListLayout.LoadDataType.HISTORY, toChatUsername, chatType);
    }
...
//        }
    ...
    public void sendImageMessage(Uri imageUri) {
        ...
        try {
            sendOriginalImage = getResources().getBoolean(com.hyphenate.easeui.R.bool.ease_enable_send_origin_image);
    ...
    public void deleteMessage(EMMessage message) {
        ...
        EmMsgManager
                .deleteMsg(message.getMsgId(), message.conversationId(), new EMCallBack() {
                    @Override
                    ...
                    public void onError(int code, String error) {
                        Log.d("EaseIM", "code=" + code + "error=" + error);
                    }
                });
    }
    ...
    public boolean onPressToSpeakBtnTouch(View v, MotionEvent event) {
        ...
        return voiceRecorder.onPressToSpeakBtnTouch(v, event, new EaseVoiceRecorderView.EaseVoiceRecorderCallback() {
            ...
            public void onVoiceRecordComplete(String voiceFilePath, int voiceTimeLength) {
                HashMap<String, Object> map = new HashMap<>();
                map.put("path", voiceFilePath);
                map.put("length", voiceTimeLength);
                sendMsgListener.sendAlert(map, 0);
            }
        });
    }
    ...
    public void sendReadAck(EMMessage message) {
        if (EaseIM.getInstance().getConfigsManager().enableSendChannelAck()) {
            ...
            if (message.direct() == EMMessage.Direct.RECEIVE
                ...
                try {
                    EMLog.e("EMUnRead", "聊天界面，发送消息已读回执，会话ID：" + conversationId + "会话类型：" + chatType);
    ...
    private void setEmojiStatusHide() {
        IChatPrimaryMenu primaryMenu = inputMenu.getPrimaryMenu();
        if (primaryMenu instanceof CustomEaseChatPrimaryMenu) {
            ((CustomEaseChatPrimaryMenu) primaryMenu).setShowEmoji(false);
        }
    }
    ...
    public boolean onResendClick(EMMessage message) {
        listener.onResendClick(message);
        return true;
    }
...
//        }
    ...
    public void inputAtUsername(String username, boolean autoAddAtSymbol) {
        ...
        EaseUser user = EaseUserUtils.getGroupUserInfo(conversationId, username);
        if (user != null) {
    ...
    private void showDefaultMenu(View v, EMMessage message) {
        ...
        findItemVisible(com.hyphenate.easeui.R.id.action_chat_quote, false);
        findItemVisible(com.hyphenate.easeui.R.id.action_chat_copy, false);
        menuHelper.setOnPopupMenuItemClickListener(new EasePopupWindow.OnPopupWindowItemClickListener() {
            ...
            public boolean onMenuItemClick(MenuItemBean item) {
                ...
                if (showDefaultMenu) {
                    ...
                    if (itemId == com.hyphenate.easeui.R.id.action_chat_copy) {
                        clipboard.setPrimaryClip(ClipData.newPlainText(null,
                                ((EMTextMessageBody) message.getBody()).getMessage()));
                        EMLog.i(TAG, "copy success");
                    } else if (itemId == com.hyphenate.easeui.R.id.action_chat_delete) {
                        deleteMessage(message);
                        EMLog.i(TAG, "currentMsgId = " + message.getMsgId() + " timestamp = " + message.getMsgTime());
                    } else if (itemId == com.hyphenate.easeui.R.id.action_chat_recall) {
                        recallMessage(message);
                    } else if (itemId == com.hyphenate.easeui.R.id.action_chat_hide) {
    ...
    private void setMenuByMsgType(View v, EMMessage message) {
        EMMessage.Type type = message.getType();
        menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_copy, false);
        menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_recall, false);
        menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_translate, false);
        menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_reTranslate, false);
        menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_hide, false);
        menuHelper.findItem(com.hyphenate.easeui.R.id.action_chat_delete).setTitle(getContext().getString(com.hyphenate.easeui.R.string.action_delete));
        ...
        switch (type) {
            case TXT:
                EMTranslationResult result = EMClient.getInstance().translationManager().getTranslationResult(message.getMsgId());
                if (v.getId() == com.hyphenate.easeui.R.id.subBubble && result != null) {
                    menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_delete, false);

                    if (result.translateCount() < 2)
                        menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_reTranslate, true);

                    menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_hide, true);
                } else {
                    menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_copy, true);
                    menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_recall, true);
                    menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_delete, true);

                    if (showTranslation(message))
                        menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_translate, true);
                }
                break;
            case LOCATION:
            case FILE:
            case IMAGE:
                ...
                break;
            case VOICE:
                menuHelper.findItem(com.hyphenate.easeui.R.id.action_chat_delete).setTitle(getContext().getString(com.hyphenate.easeui.R.string.delete_voice));
                menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_recall, true);
                break;
            case VIDEO:
                menuHelper.findItem(com.hyphenate.easeui.R.id.action_chat_delete).setTitle(getContext().getString(com.hyphenate.easeui.R.string.delete_video));
                menuHelper.findItemVisible(com.hyphenate.easeui.R.id.action_chat_recall, true);
                break;
        }
    ...
    public interface SendMsgListener {
        void sendAlert(Map<String, Object> map, int type);

        void sendGifEmoji(Object emjicon);
    }

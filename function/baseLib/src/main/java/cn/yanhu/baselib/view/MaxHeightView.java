...
public class MaxHeightView extends FrameLayout {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs,
                cn.yanhu.baselib.R.styleable.MaxHeightView);

        final int count = a.getIndexCount();
        for (int i = 0; i < count; ++i) {
            int attr = a.getIndex(i);
            if(attr == cn.yanhu.baselib.R.styleable.MaxHeightView_mhv_HeightRatio){
                mMaxRatio = a.getFloat(attr, DEFAULT_MAX_RATIO);
            }else if(attr == cn.yanhu.baselib.R.styleable.MaxHeightView_mhv_HeightDimen){

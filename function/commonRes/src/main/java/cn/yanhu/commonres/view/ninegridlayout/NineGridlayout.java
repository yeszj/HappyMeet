...
public class NineGridlayout extends ViewGroup {
    ...
    public NineGridlayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.getResources().obtainAttributes(attrs, R.styleable.NineGridlayout);
        columnCount = typedArray.getInt(R.styleable.NineGridlayout_lineCount, -1);
        mSpacing = typedArray.getDimension(R.styleable.NineGridlayout_img_spacing, DEFAULT_SPACING);
        isShowVideo = typedArray.getBoolean(R.styleable.NineGridlayout_showVideo, true);
        typedArray.recycle();
    }
...
//            mItemAspectRatio = AppUtil.getAspectRatio(imageUrls.get(0));
        ...
        for (int i = 0; i < imageUrls.size(); i++) {
            NineGridImageView view;
            String url = imageUrls.get(i);
//            if (isShowVideo && url.endsWith("mp4")) {
//                view = new NineGridImageView(getContext());
//                ((NineGridImageView) view).setOriginImageUrl(url);
//            } else {
            view = new NineGridImageView(getContext());
            ...
            view.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    nineGridImgListener.onCLick(finalI, list);
                }
            });
        }
    }

    private NineGridImgListener nineGridImgListener;

    public void setNineGridImgListener(NineGridImgListener nineGridImgListener) {
        this.nineGridImgListener = nineGridImgListener;
    }

    public interface NineGridImgListener {
        void onCLick(int position, List<ImageView> imageViewList);
    }

 ...
 * 会话列表
 */
public class CustomEaseConversationListLayout extends EaseBaseLayout implements CustomIConversationListLayout, IConversationStyle
        , IEaseConversationListView, IPopupMenu {
    public EaseRecyclerView rvConversationList;

    private ConcatAdapter adapter;
    private CustomEaseConversationListAdapter listAdapter;
    ...
    public CustomEaseConversationListLayout(Context context) {
        this(context, null);
    }

    public CustomEaseConversationListLayout(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CustomEaseConversationListLayout(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//            setModel.setContentTextSize(contentTextSize);
            ...
            float itemHeight = a.getDimension(R.styleable.EaseConversationListLayout_ease_con_item_height, dip2px(context, 0));
    ...
    private void initViews() {
        ...
        RecyclerView.ItemAnimator itemAnimator = rvConversationList.getItemAnimator();
        ...
        listAdapter = new CustomEaseConversationListAdapter();
    ...
    public void init(int conversationType) {
        this.conversationType = conversationType;
        listAdapter.setListType(false);
//        listAdapter.addDelegate(new EaseSystemMsgDelegate(setModel));
        listAdapter.addDelegate(new CustomEaseConversationDelegate(setModel));
    ...
    private void showDefaultMenu(View view, int position, EaseConversationInfo info) {
        ...
        menuHelper.setOnPopupMenuItemClickListener((item, menuPos) -> {
            ...
            if (itemId == R.id.action_con_make_read) {
                ...
                return true;
            } else if (itemId == R.id.action_con_make_top) {//置顶
                if (info.getInfo() instanceof EMConversation) {
                    ...
                    for (EMConversation emConversation : allConversationsBySort) {
                        if (emConversation.conversationId().equals(((EMConversation) info.getInfo()).conversationId())) {
                            isexist = true;
                            break;
                        }
                    }

                    EMClient.getInstance().chatManager().asyncPinConversation(((EMConversation) info.getInfo()).conversationId(), true, new EMCallBack() {
                        @Override
                ...
                return true;
            } else if (itemId == R.id.action_con_cancel_top) {//取消置顶
                if (info.getInfo() instanceof EMConversation) {
                    EMClient.getInstance().chatManager().asyncPinConversation(((EMConversation) info.getInfo()).conversationId(), false, new EMCallBack() {
                        @Override
                        public void onSuccess() {
                ...
                EMClient.getInstance().chatManager().deleteConversationFromServer(((EMConversation) info.getInfo()).conversationId(), ((EMConversation) info.getInfo()).getType(), true, new EMCallBack() {
                    @Override
                    public void onSuccess() {
...
//    }
    ...
    @Override
    public CustomEaseConversationListAdapter getListAdapter() {
        return listAdapter;
    }
